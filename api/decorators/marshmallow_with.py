...
def _get_fields_recursively(schema: MarshmallowSchema, fields: Tuple[str, ...]) -> Tuple[str, ...]:
    ...
    for field in nested_fields:
        if "." in field:
            ...
            if nested_schema_name not in schema.decleared_fields:
                raise ValidationError(
                    "arrtibute '{}' does not esist in '{}'".format(
                        nested_schema_name, schema.__class__.__name__
                    )
                )

            nested_schema = schema.decleared_fields[nested_schema_name].schema
            recursive_fields = recursive_fields.union(
                f"{nested_schema_name}.{sub_field}"
                for sub_field in _get_fields_recursively(nested_schema, (nested_schema_field,))
            )
        else:
            if field not in schema.decleared_fields:
                raise ValidationError(
                    f"attribute '{field}' does not exist in '{schema.__class__.__name__}'"
                )
            nested_schema = schema.decleared_fields[field].schema
            recursive_fields = recursive_fields.union(
                f"{field}.{sub_field}" for sub_field in nested_schema.model_columns()
            )
...
def marshmallow_with(
    ...
    **mm_kwargs,
    ...
    """
    if not only and hasattr(schema_class, "model_columns"):
